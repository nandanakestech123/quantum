...
export class ListStocksRTNClass extends React.Component {
  state = {
    headerDetails: [
      { label: "Date" },
      { label: "Doc Number" },
  ...
  getRTN = () => {
    let { RTNList, pageMeta, formField, page, limit, statusOption, empOption } =
      this.state;
    ...
    this.props
      .getCommonApi(
        `vgrnlist/?searchfrom=${dateFormat(
          From,
          "yyyy-mm-dd"
      ...
      .then(async res => {
        ...
        await this.setState({ RTNList: [] });
        RTNList = res.data.dataList;
        // pageMeta = res.data.meta.pagination;
        pageMeta = res.data.pagination;
        // pageMeta = {per_page:5, current_page:1, total:2, total_pages:4}
        this.setState({ RTNList, pageMeta });
        // this.setState({ RTNList });
  ...
  render() {
    ...
    return (
      <div className="RTN-section col-md-11 align-items-center">
        <div className="col-md-12 d-flex">
          <p className="label-head mb-4">{t("Stocks RTN Listing")}</p>
        </div>
        ...
        <div className="d-flex flex-wrap m-0 filter">
          ...
          <div className="col-md-2 col-12">
            ...
            <NormalButton
              ...
              mainbg={true}
              className="confirm"
              label="Add Stocks RTN"
              outline={false}
              onClick={() => history.push("inventory/stocksrtn/add")}
            />
          </div>
        </div>
                  ...
                  ? RTNList.map((item, index) => {
                      ...
                      return (
                        <tr key={index}>
                          ...
                          <td>
                            <div className="d-flex align-items-center justify-content-center">
                              <Link to={`inventory/stocksrtn/${PO_ID}/edit`}>
                                {DOC_NO}
                              </Link>
                            </div>
                          </td>
...
export const ListStocksRTN = withTranslation()(
  connect(mapStateToProps, mapDispatchToProps)(ListStocksRTNClass)
);
