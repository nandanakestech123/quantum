...
export class ListStocksReportClass extends React.Component {
  state = {
    headerDetails: [
      { label: "Date" },
      { label: "DOC Number" },
  ...
  getReport = () => {
    ...
    this.props
      .getCommonApi(
        `stocksheetlist/?searchfrom=${dateFormat(
          From,
          "yyyy-mm-dd"
      ...
      .then(async res => {
        ...
        await this.setState({ ReportList: [] });
        ReportList = res.data.dataList;
        // pageMeta = res.data.meta.pagination;
        pageMeta = res.data.pagination;
        // pageMeta = {per_page:5, current_page:1, total:2, total_pages:4}
        this.setState({ ReportList, pageMeta });
  ...
  render() {
    ...
    return (
      <div className="report-section col-md-11 align-items-center">
        <div className="col-md-12">
          <p className="label-head mb-4">{t("Stock Report")}</p>
        </div>
        ...
        <div className="d-flex flex-wrap m-0 filter">
          ...
          <div className="col-md-2 col-12">
            ...
            <NormalButton
              ...
              label="Add Report"
              outline={false}
              onClick={() => history.push("inventory/stockreport/add")}
            />
          </div>
        </div>
                  ...
                  ? ReportList.map((item, index) => {
                      ...
                      return (
                        <tr key={index}>
                          ...
                          <td>
                            <div className="d-flex align-items-center justify-content-center">
                              <Link to={`inventory/stockreport/${PO_ID}/edit`}>
                                {DOC_NO}
                              </Link>
                            </div>
                          </td>
...
export const ListStockReport = withTranslation()(
  connect(mapStateToProps, mapDispatchToProps)(ListStocksReportClass)
);
