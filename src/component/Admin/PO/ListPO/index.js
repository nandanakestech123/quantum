...
export class ListPOClass extends React.Component {
  state = {
    headerDetails: [
      { label: "Date" },
      { label: "PO Number" },
  ...
  getPO = () => {
    let { POList, pageMeta, formField, page, limit } = this.state;
    let { fromDate, toDate, custName, PONumber, projectName, status } =
      formField;
    ...
    this.props
      .getCommonApi(
        `polist/?searchfrom=${dateFormat(
          ...
          To,
          "yyyy-mm-dd"
        )}&searchtitle=${projectName}&searchstatus=${status}&searchname=${custName}&searchnumber=${PONumber}&page=${page}&limit=${limit}`
      )
      .then(async res => {
  ...
  render() {
    let { headerDetails, pageMeta, POList, formField } = this.state;
    ...
    return (
      <div className="project-section col-md-10 align-items-center">
        <div className="col-md-12 d-flex">
          <p className="label-head mb-4">{t("PO Listing")}</p>
        </div>
        ...
        <div className="row m-0 filter">
          ...
          <div className="col-md-2 col-12">
            ...
            <NormalButton
              ...
              label="Add PO"
              outline={false}
              onClick={() => this.props.history.push("po/add")}
            />
          </div>
        </div>
                  ...
                  ? POList.map((item, index) => {
                      ...
                      return (
                        <tr key={index}>
                          ...
                          <td>
                            <div className="d-flex align-items-center justify-content-center">
                              <Link to={`po/${id}/editPO`}>{po_number}</Link>
                            </div>
                          </td>
                          <td>
                            <div className="d-flex align-items-center justify-content-center">
...
export const ListPO = withTranslation()(
  connect(null, mapDispatchToProps)(ListPOClass)
);
