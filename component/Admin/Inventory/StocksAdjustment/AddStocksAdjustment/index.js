...
import PDF from "../../StocksPDF/Adjustment/index";

export class AddStocksAdjustmentClass extends Component {
  state = {
    formFields: {
      DOC_NO: "",
      DOC_REF1: "",
  ...
  getEmp = async () => {
    ...
    empOption = [];
    await this.props.getCommonApi(`employeelist`).then(res => {
      console.log("res.data", res);
      for (let key of res.data) {
        empOption.push({
          value: key.Emp_no,
  ...
  getAutofillItemDetails = DOC_NO => {
    this.props
      .getCommonApi(`stockadjitem/?searchdocno=${DOC_NO}&limit=10000`)
      .then(res => {
  ...
  autoFillForm = () => {
    ...
    this.props
      .getCommonApi(`stockadjlist/?searchdocid=${this.props.match.params.id}`)
      .then(async res => {
  ...
  handleItemDetailsSubmit = resADJ => {
    ...
    for (let item of itemListBeforeEdit) {
      ...
      if (!found) {
        this.props
          .commonDeleteApi(`stockadjitem/${item.item_id}/`)
          .then(res => {});
      }
    }
    ...
    for (let item of storedItemList) {
      ...
      this.props
        .getCommonApi(
          `stockadjitem/?searchdocno=${resADJ.data.DOC_NO}&searchitemcode=${item.item_code}&searchuom=${item.UOM_DESC}`
        )
        .then(resGetItem => {
          if (resGetItem.status == 200) {
            ...
            this.props
              .commonUpdateApi(
                `stockadjitem/${resGetItem.data.dataList[0].DOC_ID}/`,
                formDataItem
              )
              .then(resUpdateItem => {
            ...
            formDataItem.append("DOC_NO", resADJ.data.DOC_NO);
            this.props
              .commonCreateApi(`stockadjitem/`, formDataItem)
              .then(resCreateItem => {
  ...
  handleAddressSubmit = resADJ => {
    ...
    this.props
      .getCommonApi(`poaddr/?searchpoaddrid=${resADJ.data.id}`)
      .then(resGetAddr => {
        if (resGetAddr.status == 200) {
          ...
          formDataAddress.append("fk_po", resADJ.data.id);
          this.props
    ...
    try {
      if (this.validator.allValid()) {
        ...
        if (this.props.match.params.id) {
          ...
          var resADJ = await this.props.commonUpdateApi(
            `stockadjlist/${this.props.match.params.id}/`,
            formData
          );
          ...
          // var resADJ = await this.props.createPO(formData);

          var resADJ = await this.props.commonCreateApi(
            `stockadjlist/`,
            formData
          );

          console.log("resADJ in createPO", resADJ);
          console.log("resADJ.data.id", resADJ.data.id);
  ...
  handleChangeCost = ({ target: { value, name } }) => {
    ...
    if (r.test(value) == true || !value) {
      ...
      console.log("detailsList in handleChange after setstate", detailsList);
    } else {
      console.log("detailsList in handleChange", detailsList);
      return;
    }
  ...
  handleSelect = (
    ...
    console.log(
      "QTY + parseFloat(item_quantity)",
      QTY + parseFloat(item_quantity)
    );
    console.log("typeof QTY", typeof QTY);
    console.log(
      "typeof parseFloat(item_quantity)",
      typeof parseFloat(item_quantity)
    );

    if (this.props.tokenDetail.site_code == "HQ") {
      if (QTY + parseFloat(item_quantity) < 0) {
        Toast({
          type: "error",
    ...
    for (let list in storedItemList) {
      if (this.props.tokenDetail.site_code == "HQ") {
        if (
          ...
          Toast({
            type: "error",
            message: "his item cannot be saved, quantity exceeds limit!",
          });
          return;
        }
      }
  ...
  render() {
    ...
    let { t } = this.props;
    return (
      <div className="px-5  create-adjustment">
        <div className="head-label-nav">
          ...
          <p className="sub-category">
            {t(
              `${this.props.match.params.id ? "Edit" : "New"} Stocks Adjustment`
            )}
          </p>
        </div>
        <div className="PO-detail">
          <div className="form-group mb-4 pb-2">
            ...
            <div className="col-md-12 adjustment-content">
              <div className="py-4">
...
export const AddStocksAdjustment = withTranslation()(
  connect(mapStateToProps, mapDispatchToProps)(AddStocksAdjustmentClass)
);
